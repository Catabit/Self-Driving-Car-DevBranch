...
#define DRIVER_NAME "HLSvdmadriver"
...
{
	printk("HLSvdmadriver interrupt\n");
	return IRQ_HANDLED;
}
...
static int vdmaDriver_probe(struct platform_device *pdev)
{
	...
	if (!lp) {
		dev_err(dev, "Cound not allocate HLSvdmadriver device\n");
		return -ENOMEM;
	}
	lp->this = THIS_MODULE;
	...
	if (!lp->base_addr) {
		dev_err(dev, "HLSvdmadriver: Could not allocate iomem\n");
		rc = -EIO;
		goto error2;
	}
	...
	if (!lp->chardev){
		dev_err(dev, "HLSvdmadriver: Could not allocate chardev\n");
		rc = -EIO;
		goto error3;
	}
	...
	if (!lp->vdmactrl){
		dev_err(dev, "HLSvdmadriver: Could not allocate vdmactrl\n");
		rc = -EIO;
		goto error3;
	}
	...
	if(r_irq){
		...
		if (rc) {
			dev_err(dev, "HLSvdmadriver: Could not allocate interrupt %d.\n",lp->irq);
			goto error4;
		}
	}

	if(vdma_setup(lp->vdmactrl)<0){
		dev_err(dev, "HLSvdmadriver: Could not setup vdmactrl.\n");
		goto error4;
	}

	dev_info(dev,"HLSvdmadriver at 0x%08x mapped to 0x%08x, irq=%d\n",
