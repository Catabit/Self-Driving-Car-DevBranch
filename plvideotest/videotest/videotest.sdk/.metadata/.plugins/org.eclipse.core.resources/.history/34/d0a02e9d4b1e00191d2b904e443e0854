...
#ifdef IMAGEDUMP
#ifdef BURSTDUMP
    for (int nrImage=0; nrImage<5; nrImage++){
		printf("\n##############\nFB1 full dump %d\n#############\n", nrImage);
		...
		for (int line=0; line<handle.height; line++){
			...
			{
				uint8_t r, g, b;
				unsigned int pixel = (handle.fb3VirtualAddress[(line*(handle.width)*(handle.pixelLength)) + col+3] <<24) +
									(handle.fb3VirtualAddress[(line*(handle.width)*(handle.pixelLength)) + col+2] <<16) +
									(handle.fb3VirtualAddress[(line*(handle.width)*(handle.pixelLength)) + col+1] <<8) +
									handle.fb3VirtualAddress[(line*(handle.width)*(handle.pixelLength)) + col+0];
