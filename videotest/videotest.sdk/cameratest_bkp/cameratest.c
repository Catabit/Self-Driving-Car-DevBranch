...
int vdma_setup(vdma_handle *handle, unsigned int baseAddr, int width, int height, int pixelLength, unsigned int fb1Addr, unsigned int fb2Addr, unsigned int fb3Addr) {
    ...
    memset(handle->fb1VirtualAddress, 128, handle->width*handle->height*handle->pixelLength);
    memset(handle->fb2VirtualAddress, 128, handle->width*handle->height*handle->pixelLength);
    memset(handle->fb3VirtualAddress, 128, handle->width*handle->height*handle->pixelLength);
    return 0;
}
...
int main() {
    ...
    vdma_setup(&handle, CONTROL_REG, WIDTH, HEIGHT, 4, FB1_START, FB2_START, FB3_START);
    ...
    for(i=0; i<1; i++) {
        ...
        /*printf("FB1:\n");
		for (j = 0; j < 128; j++) printf(" %d", handle.fb1VirtualAddress[j]);
		printf("\n");

		printf("FB2:\n");
		for (j = 0; j < 128; j++) printf(" %d", handle.fb2VirtualAddress[j]);
		printf("\n");

		printf("FB3:\n");
		for (j = 0; j < 128; j++) printf(" %d", handle.fb3VirtualAddress[j]);
		printf("\n");*/
		usleep(100000);
...
/*
#ifdef IMAGEDUMP
    for (int nrImage=0; nrImage<4; nrImage++){
		printf("\n##############\nFB1 full dump %d\n#############\n", nrImage);

		while (handle.fb1VirtualAddress[128] ==128);
		printf("Sync\n");
		...
		fclose(outimg);
		vdma_start_triple_buffering(&handle);
		//vdma_set(&handle, OFFSET_VDMA_S2MM_CONTROL_REGISTER, VDMA_CONTROL_REGISTER_START);
		vdma_s2mm_status_dump(&handle);
		sleep(2);
...
*/
	...
	char filename[100];
	sprintf(filename, "/home/root/outimg.ppm");
	FILE *outimg = fopen(filename, "w");
	fprintf(outimg, "P3\n%d %d\n%d\n", WIDTH, HEIGHT, 255);
